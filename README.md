...
選擇 Log 類型（Windows / Linux / Application）
...
按下「使用 AI 進行智能分析」，查看 Markdown 分析報告

🧠 Log 解析與 AI 分析設計
Log 解析（services/parser.py）
根據選擇的 log_type 使用不同 regex 規則

將每一行 Log 轉換為 dict，再組成 pandas DataFrame

解析失敗的行會被略過，避免影響整體結構

AI 分析（services/analyzer.py）
將 DataFrame 轉換為適合 LLM 閱讀的文字格式（含 CSV 預覽）

若紀錄過多，只取前 200 筆作為分析樣本，並提示已截斷

系統提示（system prompt）將模型設定為「資深系統工程師 / SRE」
...
最終結果以 Markdown 格式回傳，方便直接複製到報告或簡報

📌 未來可延伸方向（Roadmap）
對 Linux syslog 自動推斷 level（例如依據關鍵字判定 ERROR / INFO）

支援更多 Log 格式（IIS、Nginx、應用程式 JSON log 等）

整合簡易 Alert 規則（例如某錯誤次數超過門檻時給出警示）

與現有監控／工單系統串接（如 Prometheus、ELK、ServiceNow、JIRA）

增加圖表視覺化（Error 趨勢、Top source / process 等）
